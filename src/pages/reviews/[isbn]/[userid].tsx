...
async function getPostData(isbn: number, userId: number): Promise<PostData | null> {
...
export async function getServerSideProps({ params }: GetStaticPropsContext) {
	if (!params || !params.isbn || typeof params.isbn !== 'string' || isNaN(parseInt(params.isbn))) {
		return {
			props: {
				postData: null,
			},
	...
	if (!params || !params.userid || typeof params.userid !== 'string' || isNaN(parseInt(params.userid))) {
		return {
			props: {
				postData: null,
			},
	...
	const postData = await getPostData(parseInt(params.isbn), parseInt(params.userid));
